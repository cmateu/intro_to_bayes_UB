...
for err_pii in [0.005,0.1]:
 ...
 #for N in [200,]:
  ...
  ax.hist(pi,histtype='step',normed=True,lw=3.)
  ax.set_xlim(-0.3,0.3)
  if ax.is_last_row(): ax.set_xlabel('$\pi$')
  if ax.is_first_col(): ax.set_ylabel('$N(\pi)$')
  if ax.is_first_row(): 
   ax.set_title('$\langle 1/\pi_k \\rangle=%.2f$, $1/\langle\pi_k \\rangle=%.2f$' % (np.mean(1./pi),1./(np.mean(pi))))
